...
class ScriptBuilderTestCase(unittest.TestCase):
    ...
    def test_emit_push_uint(self):
        sb = vm.ScriptBuilder()
        sb.emit_push(types.UInt160.from_string("0x6f1837723768f27a6f6a14452977e3e0e264f2cc"))
        # captured from C#
        expected = "0c14ccf264e2e0e3772945146a6f7af268377237186f"
        self.assertEqual(expected, sb.to_array().hex())

    def test_emit_push_public_key(self):
        key_raw = bytes.fromhex("03b209fd4f53a7170ea4444e0cb0a6bb6a53c2bd016926989cf85f9b0fba17a70c")
        pub_key = cryptography.ECPoint.deserialize_from_bytes(key_raw)
        sb = vm.ScriptBuilder()
        sb.emit_push(pub_key)
        # captured from C#
        expected = "0c2103b209fd4f53a7170ea4444e0cb0a6bb6a53c2bd016926989cf85f9b0fba17a70c"
        self.assertEqual(expected, sb.to_array().hex())
    ...
    def test_emit_push_bigger_numbers(self):
        ...
        self.assertEqual("Input number exceeds maximum data size of 32 bytes", str(context.exception))
    ...
    def test_emit_jump(self):
        ...
        self.assertEqual("OpCode DUP is not a valid jump OpCode", str(context.exception))
