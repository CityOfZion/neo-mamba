import unittest
from neo3.contracts import descriptor
...
class ContractPermissionDescriptorTestCase(unittest.TestCase):
    ...
    def test_wildcard(self):
        cpd = descriptor.ContractPermissionDescriptor()
        self.assertTrue(cpd.is_wildcard)
        self.assertDictEqual({'contract': '*'}, cpd.to_json())
        cpd_from_json = descriptor.ContractPermissionDescriptor.from_json(cpd.to_json())
        self.assertEqual(cpd.contract_hash, cpd_from_json.contract_hash)
        self.assertEqual(cpd.group, cpd_from_json.group)

    def test_group(self):
        cpd = descriptor.ContractPermissionDescriptor(contract_hash=types.UInt160.zero())
    ...
    def test_contract_hash(self):
        cpd = descriptor.ContractPermissionDescriptor(group=self.keypair.public_key)
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(ValueError) as context:
            descriptor.ContractPermissionDescriptor.from_json({'contract':None})
        self.assertEqual("Invalid JSON - Cannot deduce permission type from None", str(context.exception))

        with self.assertRaises(ValueError) as context:
            descriptor.ContractPermissionDescriptor.from_json({'contract': 'abc'})
