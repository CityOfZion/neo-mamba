...
async def example_airdrop(neoxp: shared.NeoExpress):
    ...
    neo = NeoToken()
    print("Minting..", end='')
    print(await facade.invoke(neo.transfer(source=account.address, destination=shared.coz_token_hash, amount=100)))
    print(await facade.invoke(neo.transfer(source=account.address, destination=shared.coz_token_hash, amount=100)))
    ...
    destination_addresses = ["NWuHQdxabXPdC6vVwJhxjYELDQPqc1d4TG", "NhVnpBxSRjkScZKHGzsEreYAMS1qRrNdaH", "NanYZRm6m6sa6Z6F3RBRYSXqdpg5rZqxdZ","NUqLhf1p1vQyP2KJjMcEwmdEBPnbCGouVp","NKuyBkoGdZZSLyPbJEetheRhMjeznFZszf"]
    print("Airdropping 10 tokens and waiting for receipt")
    print(await facade.invoke(token.transfer_multi(account.address, destination_addresses, 10)))


if __name__ == "__main__":
    with shared.NeoExpress(shared.neoxpress_config_path, shared.neoxpress_batch_path) as neoxp:
