...
# This initialises the local chain with the genesis block and allows us to get a snapshot of the databse. This is
# required for calculating network fees automatically. Always call this AFTER setting the network settings or network
# syncing and other parts of the system will fail.
bc = blockchain.Blockchain()
snapshot = bc.currentSnapshot

# We start with adding a wallet with a multi-signature account that we will need to sign the transaction (as well as
# obtain the address for to specify as source funds).

# There are different means of creating/importing a wallet choose your flavour. We will use the most manual one
# create a wallet and add the consensus node account (address) where the key is protected by the password "123"
w = wallet.Wallet()
...
# Next add the consensus node multisig address to wallet
...
#
# In the single consensus node setup we onle to sign with one key, thus we have a threshold of 1 and just 1 public key
...
#
# It's signature on the native side looks as follows:
...
# Note that if the first parameter of a native contract is `engine` or `snapshot` then it can be ignored. This thus
...
# Abitrary additional data to supply that will be printed in the "transfer" notify event.
data = None

sb = vm.ScriptBuilder()
sb.emit_dynamic_call_with_args(contracts.NeoToken().hash, "transfer", [from_account, to_account, amount, data])

# With our script done we need to create a Transaction and add the script

tx = payloads.Transaction(version=0,
                          nonce=123,
                          system_fee=0,
                          network_fee=0,
                          valid_until_block=1500,
                          attributes=[],
                          signers=[],
                          script=sb.to_array())
...
# or adding to little. On a private network this isn't a problem, but on the real chain you'd like to be precise.
...
# Because there is a threshold of just 1 this call will automatically add the witness to the tx
account.sign_multisig_tx(tx, "123", ctx)
# We had a 3 out of 4 threshold, we's imply call `sign_multisig_tx` multiple times with each required account.
# The context has properties/methods to query which keys still need to sign and whether sufficient accounts have signed
# the tx.


# Add this point we're done with signing and the transaction is ready to be relayed to the network. There are 2 options
...
#     This option requires being connected over P2P to the private network.
...
async def sync_network():
    ...
    # calculation of the transaction fees leading to rejection of the transaction.
