...
async def main(
    ...
    sb.emit_contract_call_with_args(
        contract.CONTRACT_HASHES.MANAGEMENT, "deploy", [nef_bytes, manifest_bytes]
    )
    ...
    async with api.NeoRpcClient(rpc_host) as client:
        try:
            ...
            print(
                f"Contract hash: {contractutils.get_contract_hash(tx.sender, nef_.checksum, manifest_.name)}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", metavar="CONTRACT_NEF", required=True, help="contract .NEF path"
    )
    parser.add_argument(
        "-m",
        metavar="CONTRACT_MANIFEST",
        required=True,
        help="contract .manifest.json path",
    )
    parser.add_argument(
        "-s",
        metavar="RPC_SERVER",
