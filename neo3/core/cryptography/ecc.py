...
class EllipticCurve:
    ...
    class ECPoint(serialization.ISerializable):
        """
        Represents a value in the EllipticCurve
        ...
        def deserialize(self, reader: serialization.BinaryReader):
            # note: only works with compressed points. This is what NEO expects (see serialize())
            point = ECDSA.deserialize_secp256r1(reader)
            self.x = point.x
            self.y = point.y
...
class ECDSA:
    ...
    def sign(self, message, privkey, secret):
        ...
        Returns:
            (G*k,  (m+x*r)/k), for signsecret k, message m, privatekey x
        """
    ...
    def verify(self, message, pubkey, rnum, snum) -> bool:
        """
    ...
    def findpk(self, message, rnum, snum, flag):
        ...
        Note: there are 2 pubkeys related to a signature.
        """
        m = self.GFn.value(message)
        r = self.GFn.value(rnum)
        s = self.GFn.value(snum)
    ...
    def secp256r1() -> ECDSA:
        ...
        return ECDSA(ec,
                     ec.point(0x6B17D1F2E12C4247F8BCE6E563A440F277037D812DEB33A0F4A13945D898C296,
                              0x4FE342E2FE1A7F9B8EE7EB4A7C0F9E162BCE33576B315ECECBB6406837BF51F5),
                     GFp)

    @staticmethod
    def decode_secp256r1(str, unhex=True, check_on_curve=True) -> ECDSA:
        """
    ...
    @staticmethod
    def deserialize_secp256r1(reader: serialization.BinaryReader) -> EllipticCurve.ECPoint:
        GFp = FiniteField(int("FFFFFFFF00000001000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF", 16))
        ec = EllipticCurve(GFp, 115792089210356248762697446949407573530086143415290314195533631308867097853948,
                           41058363725152142129326129780047268409114441015993725554835256314039467401291)
    ...
    @staticmethod
    def sign_secp256r1(message, prikey, pubkey: EllipticCurve.ECPoint):
